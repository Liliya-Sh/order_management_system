...
# from restaurant_ordering_system.settings import CART_SESSION_ID
...
class Cart:
    ...
    def add(self, product, quantity=1, override_quantity=False):
        ...
        if override_quantity:
            ...
            self.cart[product_id]['quantity'] += quantity
        print(f"Добавить продукта {product.name_product} в корзину. Количество: {self.cart[product_id]['quantity']}")
        self.save()
    ...
    def save(self):
        ...
        if settings.CART_SESSION_ID in self.session:
            print("Session modified:", self.session[settings.CART_SESSION_ID])
        else:
            print("Cart session ID not found.")
